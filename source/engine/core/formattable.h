...
template <class T> string to_string(T value) { return std::to_string(value); }

template <> inline string to_string(string s) { return s; }
...
template <class T> T from_string(string s) { return T{}; }

template <> inline string from_string(string s) { return s; }

template <> inline string_view from_string(string s) { return s; }
...
template <> inline linef from_string(string s) {
    ...
    return linef{from_string<vec2f>(s.substr(0, comma_pos)),
                 from_string<vec2f>(s.substr(comma_pos))};
}
...
template <> inline vec4f from_string(string s) {
    ...
    if (std::regex_search(s, match, rgx)) {
        ...
        return vec4f{std::stof(match[1]), std::stof(match[2]), std::stof(match[3]),
                     std::stof(match[4])};
    }
